...
-- 모든 팀 정보 출력하기
SELECT TEAM_NO, STADIUM_NO, TEAM_NAME, TEAM_LEADER, DIRECTOR, SPONSOR, TEAM_REGION,
TEAM_DES, TEAM_ST, VIDEO_URL, TEAM_COLOR, 
       (SELECT IMG_ORIGINAL FROM TEAM_IMG 
        WHERE TEAM_IMG.TEAM_NO = TEAMS.TEAM_NO 
        AND IMG_LEVEL = 1) AS IMG_ORIGINAL1,
        (SELECT IMG_ORIGINAL FROM TEAM_IMG 
        WHERE TEAM_IMG.TEAM_NO = TEAMS.TEAM_NO 
        AND IMG_LEVEL = 2) AS IMG_ORIGINAL2,
        (SELECT IMG_ORIGINAL FROM TEAM_IMG 
        WHERE TEAM_IMG.TEAM_NO = TEAMS.TEAM_NO 
        AND IMG_LEVEL = 3) AS IMG_ORIGINAL3,
        (SELECT IMG_ORIGINAL FROM TEAM_IMG 
        WHERE TEAM_IMG.TEAM_NO = TEAMS.TEAM_NO 
        AND IMG_LEVEL = 4) AS IMG_ORIGINAL4
            
FROM TEAMS
WHERE TEAM_ST = 'Y'
AND IMG_ORIGINAL1 IS NOT NULL
;
