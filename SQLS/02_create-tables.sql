...
COMMENT ON COLUMN "PLYAER"."PLAYER_NO" IS '선수 번호';
COMMENT ON COLUMN "PLYAER"."POSITION_NO" IS '포지션 번호';
COMMENT ON COLUMN "PLYAER"."TEAM_NO" IS '구단 번호(SEQ_TEAM_NO)';
COMMENT ON COLUMN "PLYAER"."PLAYER_NAME" IS '선수 이름';
COMMENT ON COLUMN "PLYAER"."PLAYER_UNIFORM_NO" IS '선수 등번호';
COMMENT ON COLUMN "PLYAER"."PLAYER_BD" IS '생년월일(YYYYMMDD)';
COMMENT ON COLUMN "PLYAER"."JOIN_YEAR" IS '입단년도(YYYY)';
COMMENT ON COLUMN "PLYAER"."HEIGHT" IS '신장';
COMMENT ON COLUMN "PLYAER"."WEIGHT" IS '체중';
COMMENT ON COLUMN "PLYAER"."SCHOOL" IS '출신학교 (초등학교,  중학교, 고등학교, 대학교)';
COMMENT ON COLUMN "PLYAER"."SALARY" IS '연봉';
COMMENT ON COLUMN "PLYAER"."CAREER" IS '경력사항 ( ","로 구분하여 VARCHAR)';
COMMENT ON COLUMN "PLYAER"."PLAYER_ST" IS '선수 활동/ 비활동';
...
CREATE TABLE "COMM" (
	...
	CONSTRAINT FK_COMM_TEAM_NO FOREIGN KEY (TEAM_NO) REFERENCES TEAM(TEAM_NO)  ON DELETE CASCADE,
...
CREATE TABLE "COMM_IMG" (
	...
	"IMG_RENAME"	VACHAR2(30)		NOT NULL,
	"IMG_ORIGINAL"	VACHAR2(30)		NOT NULL,
	"IMG_LEVEL"	NUMBER		NOT NULL,
    CONSTRAINT PK_COMM_IMG_NO PRIMARY KEY (IMG_NO),
    CONSTRAINT FK_COMM_IMG_COMM_NO FOREIGN KEY (COMM_NO) REFERENCES COMM (COMM_NO)
);

COMMENT ON COLUMN "COMM_IMG"."IMG_NO" IS '이미지 번호(시퀀스)';
...
CREATE SEQUENCE SEQ_COMM_IMG_NO;
